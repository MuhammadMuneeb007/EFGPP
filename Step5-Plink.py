...
numberofpca = ["100"]  # Setting the number of PCA components to be included
...
if spacing == "log":
    ...
    all_pvalues = np.linspace(1e-10, 1, num_intervals, endpoint=True)
else:
    raise ValueError("Invalid spacing value. Use 'log' or 'linear'.")

range_list_path = os.path.join(save_path, f"Fold_{foldnumber}", 'range_list')
...
def transform_plink_data(traindirec, newtrainfilename,p, p1_val, p2_val, p3_val, c1_val, c2_val, c3_val,Name,pvaluefile):
    ...
    command = [
        ...
        "--score", save_path + os.sep +filedirec+"_Plink.txt", "3", "4", "9", "header",
    ...
    command = [
        ...
        "--q-score-range", traindirec+os.sep+"range_list",traindirec+os.sep+"SNP.pvalue",
        "--extract", traindirec+os.sep+trainfilename+".valid.snp",
        "--out", folddirec+os.sep+Name+os.sep+validationfilename
    ]
    subprocess.run(command)
